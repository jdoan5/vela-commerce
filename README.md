...
| Generated by | a fixed seed (`20260214`), no clock, no `Guid.NewGuid` — byte-identical between runs, and CI fails if that stops being true |
...
The HTTP surface also has an executable description as a [Bruno collection](api-tests/README.md),
plain-text `.bru` files that CI runs headless against a live API.
