...
| A **`pg_dump` backup** written to blob storage, named in the risk table as the reconstruction story | Not built, and [ADR 0010](adr/0010-the-purge-runs-on-visits-not-on-a-clock.md) explains why it stayed unbuilt rather than being forgotten. The mitigation's other half is real: the committed seed plus migrations reconstructs the database, and that path is what `--seed` exists for | §6, §9, §10 |
...
| A **monthly maintenance ritual** with a calendar reminder — click the demo, check badges and the status page, read the Neon CU-hour graph, refresh the cost table's "verified on" dates | Not written down anywhere. Dependabot runs weekly and keeps the last commit recent, which is one line of the ritual and not the rest | §6, §10 |
