...
| **xUnit v3** on Microsoft Testing Platform, **Playwright** E2E publishing `trace.zip`, a **Lighthouse** budget failing PRs, **Stryker** in the tool manifest | xUnit **2.9.3**. No browser test of any kind exists, so the CSP assertion, the hydration visual check and the performance budget the plan leans on were never written. Stryker IS in the manifest now and gates CI at a score of 70 — this row said mutation testing was done by hand, one claim at a time, and that stopped being true on 2026-09-05 | §2, §4, §6 |
| Central Package Management, `Directory.Build.props`, `.config/dotnet-tools.json`, `rollForward: latestPatch` | None adopted. The tool manifest is at the repository root and now holds three tools — `dotnet-ef`, `dotnet-reportgenerator-globaltool` and `dotnet-stryker`, not `dotnet-ef` alone as this row used to say; `global.json` still says `latestFeature` | §2, §7, §11 |
...
| **Four Azure Container Apps Jobs** on cron — nightly reset + session reaper, a reconciler safety net, the migration bundle and a `pg_dump` backup — each pinging an **UptimeRobot heartbeat** | **Zero jobs exist.** Migrations run from a `workflow_dispatch` GitHub workflow, and demo data is purged by an in-process worker while the container is awake — [ADR 0010](adr/0010-the-purge-runs-on-visits-not-on-a-clock.md) overturns this line and gives the arithmetic. The reconciler safety net and the backup are simply not built | §2, §3, §4, §6, §9 |
| **UptimeRobot Free**: a keyword monitor on the storefront, a 30-minute `/alive` check, a heartbeat on the nightly job, and a public status page linked from the README | **None of it.** No account is referenced anywhere in the repository. `/alive` and `/health` exist and are wired to the container's probes; nothing external watches either, and [ADR 0009](adr/0009-no-log-analytics-workspace.md) already records that there is no alerting of any kind | §2, §4, §5, §6, §10 |
| A **`pg_dump` backup** written to blob storage, named in the risk table as the reconstruction story | Not built, and until [ADR 0011](adr/0011-no-database-backup.md) no record said whether that was a decision or an omission. The mitigation's other half is real and now timed: the committed seed plus migrations rebuilds the database, which is what `--seed` exists for | §6, §9, §10 |
| A **committed coverage badge** (coverlet → Cobertura → ReportGenerator → badge) sitting next to a mutation-score badge on the README's first screen | Neither badge exists, deliberately. CI computes the number and fails below a 65% floor instead — `ci.yml` carries the argument, and the README says so where the badge would be. A badge asserts; a floor enforces | §2, §4, §5, §6, §10 |
| A **monthly maintenance ritual** with a calendar reminder — click the demo, check badges and the status page, read the Neon CU-hour graph, refresh the cost table's "verified on" dates | Written down in [`MAINTENANCE.md`](MAINTENANCE.md), minus the status page, which needs a monitor that does not exist. The calendar reminder is the reader's to set — the document says so rather than pretending otherwise | §6, §10 |
...
- https://learn.microsoft.com/en-us/aspnet/core/blazor/components/render-modes#static-server-side-rendering-static-ssr — `HtmlRenderer` for rendering components to static HTML outside a request (the prerendered-shell technique) (as of 2026-09-02)
...
- https://docs.github.com/en/actions/how-tos/secure-your-work/security-harden-deployments/oidc-in-aws — AWS OIDC provider, audience `sts.amazonaws.com` (as of 2026-09-02)
