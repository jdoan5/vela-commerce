...
- **Cold start is a tracked metric with an engineering budget.** Image under ~140 MB (`noble-chiseled-extra` + ReadyToRun), no migrations at startup, no API-docs UI in Production, a deliberately small DI graph, Kestrel binding the port before expensive initialization so ACA does not kill a slow starter, client retry with backoff, and an honest "waking the demo up" skeleton. Measured **locally first** by running the published image under Docker Desktop, then continuously by the 30-minute uptime check.
...
| **GitHub Container Registry** | Public packages free; container storage and bandwidth **currently free** with ≥1 month's notice before change | **Two** images (~140 MB app + a small migration bundle), a tag per commit | Public images are unmetered. Fallback: ACR Basic ~$0.167/day ≈ $5/mo (*secondary-sourced*) |
